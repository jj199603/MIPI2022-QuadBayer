...
class MIRNet(nn.Module):
    def __init__(self, in_channels=4, out_channels=4, n_feat=64, kernel_size=3, stride=2, n_RRG=3, n_MSRB=2, height=3, width=2, bias=False):
        ...
        # self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=bias)
        # self.pixelshuffle = nn.Conv2d(out_channels, 1, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=False)
        # init.normal_(self.pixelshuffle.weight, mean=0.0, std=0.001)
    ...
    def forward(self, x):
        ...
        h = self.conv_out(h)
        h += x
        # h = self.pixelshuffle(self.upsample(h))
        # h = nn.ReLU(inplace=True)(h)
        h = self.pixelshuffle(h)
        return h