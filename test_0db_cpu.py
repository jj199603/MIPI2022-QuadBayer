...
INPUT_folder = './Quad_test_dataset_fullres/input/0db/'
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
denoise_net = MIRNet(in_channels=4, out_channels=4).to(device)
pretrained = torch.load('./checkpoint/mir_0db_aug2.pt')
...
for filename in os.listdir(INPUT_folder):    
    ...
    quad_mulchannel = quad_decompose(quad_bayer_tensor).to(device) # tensor (4, 600, 900)
    ...
    # save_path_1 = './output/0db/' + img_id + '_0db_denoised.png'
    # save_path_2 = './output/0db/' + img_id + '_0db_input.png'
