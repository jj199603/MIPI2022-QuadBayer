...
#INPUT_folder = './Quad_test_dataset_fullres/input/42db/'
INPUT_folder = './Quad_test_dataset_fullres/input/42db/' 
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
# device = torch.device("cpu")
denoise_net = MIRNet(in_channels=4, out_channels=4).to(device)
pretrained = torch.load('./checkpoint/mir_42db_aug2.pt')
new_state_dict = OrderedDict()
for k, v in pretrained.items():
    name = k[7:] # remove `module.`
    new_state_dict[name] = v
denoise_net.load_state_dict(new_state_dict, strict=True)
...
for filename in os.listdir(INPUT_folder):    
    ...
    rggb_bayer = quad2rggb(quad_bayer_normal)
    r = rggb_bayer[::2, ::2]
    g1 = rggb_bayer[::2, 1::2]
    g2 = rggb_bayer[1::2, ::2]
    b = rggb_bayer[1::2, 1::2]
    rggb_bayer_split = np.stack((r, g1, g2, b))
    quad_mulchannel = torch.from_numpy(rggb_bayer_split).float().to(device)
    ...
    # quad_mulchannel = quad_decompose(quad_bayer_tensor) # tensor (4, 600, 900)
    ...
    # save_path_1 = './output/42db/' + img_id + '_42db_denoised.png'
    # save_path_2 = './output/42db/' + img_id + '_42db_input.png'
